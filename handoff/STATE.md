...
- `CURRENT_ITERATION:` 20  (awaiting PLAN_20)
- `LAST_PLAN:` PLAN_19_main-results-imuwifine.md (first iteration of the multi-iter main-results table per SCIENTIST_NOTE; IMUWiFine row: CNN1D + LSTM-attn + wlan_localization + RoNIN ResNet1D; both per-leg SOTAs are NEW measurements never run on IMUWiFine)
- `LAST_RESULT:` RESULT_19_main-results-imuwifine.md (**IMUWiFine row populated**: wlan_localization NEW val 4.17 / test 8.50 m; RoNIN ResNet1D NEW val 26.84 m / test n/a (IMUWiFine test paths lack IMU); CNN1D fusion val **1.397** / test 7.094 m; LSTM-attn fusion val **1.264** / test 7.196 m. Outcome **α'''' on VAL** (we beat both per-leg SOTAs by 70%/95%); **β'''' on TEST** (we beat WiFi SOTA by 16-17%, RoNIN unmeasurable). LSTM-attn dead-reckoning regime REPLICATES on IMUWiFine — second data point: only:imu 1.263 ≈ full 1.264 (Δ=0.1%). Smoothness r≈0 (debt persists across 3 datasets × 5 architectures). PLAN_20 recommendation = IPIN 2024 floor 0 (IMU on both splits → RoNIN row fully measurable))
- `GOAL_REACHED:` false
