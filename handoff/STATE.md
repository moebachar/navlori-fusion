...
- `CURRENT_ITERATION:` 16  (RESULT_16 committed — bake-off inconclusive on subset; awaiting PLAN_17)
- `LAST_PLAN:` PLAN_16_phase-b-architecture-bakeoff.md (4 candidates {LSTM-attn, TCN, 1D-CNN, Transformer-from-scratch} on 10 % Webots subset @ K=4 B=128; incumbent run-1 FusionTransformer benchmarked alongside on same subset for fair compare; decision: beat 0.417 m AND r > 0.20 ⇒ new winner, else incumbent stands but defensible)
- `LAST_RESULT:` RESULT_16_phase-b-architecture-bakeoff.md (2026-05-26 ~07:15 local — bake-off on 2-path subset: 3 of 4 candidates implemented (transformer_scratch cut); CNN1D val/test 0.978/1.261, LSTM-attn val/test 0.978/1.286, TCN val/test 1.009/1.288 all BEAT incumbent val/test 1.493/1.688 by 24-34 % at 1/3 params; smoothness r ≤ 0.085 (no candidate clears > 0.20 gate); strict PLAN_16 rule NOT met; recommend PLAN_17 = full-data retrain CNN1D + LSTM-attn)
- `GOAL_REACHED:` false
