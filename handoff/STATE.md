...
- `CURRENT_ITERATION:` 13  (RESULT_13 committed; **outcome α'' — C3 WINNER** at K=4 B=128 4-mod; awaiting PLAN_14)
- `LAST_PLAN:` PLAN_13_phase-b-batch-lr-probe.md (single-axis batch confound probe: B=64→B=128 with K=4 + 4-mod, lr unchanged; isolates whether batch was the regression driver since RESULT_11 halved B for K=8 memory safety)
- `LAST_RESULT:` RESULT_13_phase-b-batch-lr-probe.md (2026-05-26 ~02:10 local — **K=4 + 4-mod + B=128 val 0.394 / test 0.417**, BEATS run-1 K=8 ref 0.43 m by 9 %; staleness slope 0.417→0.929 m across 18 s (×2.2 fresh); subset shows wifi+imu+camera (drop Odom) is the actual winner at val 0.381 / test 0.406; C3 cleared with margin; recommend PLAN_14 = full ablations + Phase C kickoff)
- `GOAL_REACHED:` false
