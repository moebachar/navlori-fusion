...
- `CURRENT_ITERATION:` 22  (awaiting PLAN_22)
- `LAST_PLAN:` PLAN_21_transformer-from-scratch.md (restore the bake-off candidate dropped by PLAN_16's "cut to 3 if overrun" provision; scientist designs `MoTTransformer` from first principles using RESULTs 17/18 evidence — 3-layer 2-head transformer w/ ALiBi temporal bias, learnable modality embeddings, single-query cross-attn readout; ~0.48 M params (fair vs CNN1D 0.51 M / LSTM-attn 0.57 M); key secondary test: does ALiBi finally clear smoothness r > 0.20 gate?)
- `LAST_RESULT:` RESULT_21_transformer-from-scratch.md (**outcome γ5 — MoTTransformer is WORST of 4 architectures**: val 0.594 / test **0.608 m** (+79% vs CNN1D, +46% vs incumbent), 0.74 M params, smoothness r=0.019 (ALiBi does NOT clear 0.20 gate), staleness slope 0.028 m/s (architecture-invariant), latency b=1 5.82 ms / b=32 0.20 ms. Subset eval reveals **WiFi-anchored regime** (only:wifi 0.704 close to full; motion-only 3-5 m alone) — opposite of LSTM-attn dead-reckoning, also worse than CNN1D cooperative. ALiBi likely suppresses cross-instant motion fusion. **4-architecture bake-off complete**: CNN1D winner, LSTM-attn runner-up with dead-reckoning structural finding, incumbent over-parameterised, MoTTransformer loses. **Smoothness debt confirmed loss-function-bound across 4 archs × 2 data scales** (architectural lever falsified). PLAN_22 recommendation = continue main-results table at IPIN floor 0)
- `GOAL_REACHED:` false
