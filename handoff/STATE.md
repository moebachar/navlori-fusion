...
- `CURRENT_ITERATION:` 25  (awaiting PLAN_25 SUMMARY + table assembly; all 6 main-table rows now populated)
- `LAST_PLAN:` PLAN_24_main-results-uji-k1-degenerate.md (UJI K=1 + M=1 degenerate; SOTA numbers reused (wlan_localization 15.17 / Anchor2Vec 8.69 from RESULT_01); run CNN1D + LSTM-attn aggregators on per-scan WiFi-only; expected α7 = degenerate collapse to Anchor2Vec 8.69 ±5 %; β7 = unexpected divergence at K=1 M=1)
- `LAST_RESULT:` RESULT_24_main-results-uji-k1-degenerate.md (**outcome α7 CONFIRMED — aggregators degenerate to Anchor2Vec + head on per-scan WiFi**: CNN1D 0.22 M val **8.723 m** (+0.4% vs Anchor2Vec 8.69), LSTM-attn 0.29 M val **8.426 m** (−3.0% vs Anchor2Vec, marginally beats). Both BEAT wlan_localization SOTA 15.17 m by ~43-45%. At K=1 M=1 the aggregators have no temporal/cross-modal axis to operate on — they re-parameterise the head with a few percent of noise. **WiFi encoder Anchor2Vec is the load-bearing component on per-scan WiFi-only data.** Main-results table now has all 6 rows populated. PLAN_25 recommendation = SUMMARY + main-table assembly; includes anchored numbers + open paper-framing questions + queued PLAN_25b loss-function lever experiment)
- `GOAL_REACHED:` false
