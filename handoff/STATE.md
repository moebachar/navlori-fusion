...
- `CURRENT_ITERATION:` 24  (awaiting PLAN_24)
- `LAST_PLAN:` PLAN_23_main-results-ronin-single-mod.md (RoNIN canonical single-mod IMU row: ResNet1D 5.140 m + IMUCNN 9.961 m reused from RESULT_07; CNN1D + LSTM-attn aggregators over chunked 50-step IMU sub-windows at K=4 M=1; asks whether temporal aggregator helps over pure IMUCNN; three outcomes α6/β6/γ6)
- `LAST_RESULT:` RESULT_23_main-results-ronin-single-mod.md (**outcome β6 — aggregator helps; gap to SOTA narrows from +94% to +47-48% raw / +16-19% Umeyama**: CNN1D 0.20 M raw ATE **7.587** / Umeyama 5.945 / RTE 12.690; LSTM-attn 0.26 M raw ATE 7.497 / Umeyama 6.122 / RTE 12.606. Both improve over IMUCNN-only (9.961) by ~24%; **CNN1D Umeyama gap +15.7% CLEARS 20% audit gate**; LSTM-attn +19.1% just barely clears. **Raw ATE 20% gate not cleared by either** (audit-weighted decision per rubric correction #3 — C2 stays `keep (in-domain only)`). **RTE 3× ResNet1D** (12.6 vs 4.4) — aggregator improves global drift, worsens local consistency. The aggregator's temporal lever exists at M=1 (refutes γ6); CNN1D ≈ LSTM-attn at this regime (within 1.2%). Honest finding: RTE-to-ATE asymmetry on RoNIN + smoothness debt on Webots both point to loss-function lever (RESULT_18 B-1/B-2 hypothesis). PLAN_24 recommendation = UJI K=1 degenerate row using RESULT_01 numbers)
- `GOAL_REACHED:` false
