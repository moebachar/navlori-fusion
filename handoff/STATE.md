...
- `CURRENT_ITERATION:` 27  (awaiting PLAN_27)
- `LAST_PLAN:` PLAN_26_external-methods-relocation-and-baselines.md (foundational consolidation step: relocate 4 SOTA submodules to `external_methods/` + create `src/pipeline/baselines/` centralised loader package + migrate all wrappers to import from there; 5-iteration consolidation roadmap PLAN_26→30 documented in the plan)
- `LAST_RESULT:` RESULT_26_external-methods-relocation-and-baselines.md (**foundational consolidation shipped**: 4 SOTA repos added as Git submodules under `external_methods/` pinned to RESULT_01-24 commits; new `src/pipeline/baselines/` package wraps each baseline (`load_position_regressor`, `load_preprocessor`, `BasicBlock1D/ResNet1D/...`, `apply_tartanvo_shims`, `load_basic_encoder4/BasicEncoder4`); 4 idempotent compat shims (np.int, scipy.as_dcm, numpy.linalg.linalg, cupy.compile_with_cache) all centralised in `_shims.py` per Demand #3; 12 wrappers + 1 src file migrated (~250 lines of boilerplate deleted); `docs/EXTERNAL_DEPENDENCIES.md` written; `README.md` + `CLAUDE.md` updated. **Verification: eval_wlanloc_uji.py reproduces 15.171 m exactly = RESULT_01 0.0% drift**. PLAN_27-30 foundation in place)
...
| 29 | `src/pipeline/evaluation/MainResultsTable` (reads RESULT JSONs, returns paper-table DataFrame) + canonical `scripts/eval_*.py` triage + configs/docs sweep | 26, 27, 28 |
| 30 | `notebooks/run2_walkthrough.ipynb` scaffold (§0 dataset pre-section with stats + figures + preprocessing demos; §1-3 Phase A/B/C walkthrough; §4 honest gaps; §5 paper-framing decisions; §6 reproducibility) — engineer iterates with user directly after this | 26, 27, 28, 29 |
