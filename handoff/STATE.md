...
- `CURRENT_ITERATION:` 28  (awaiting PLAN_28)
- `LAST_PLAN:` PLAN_27_data-factory-and-visualization.md (data side of notebook §0 pre-section: per-dataset loader modules with uniform `load/stats/preprocessing_demo` API across 7 datasets + dispatcher factory + `src/pipeline/visualization/` plotter package; smoke verification = 7 dataset-overview PNGs)
- `LAST_RESULT:` RESULT_27_data-factory-and-visualization.md (**data + viz packages shipped**: uniform `src.pipeline.data.{load_dataset, dataset_stats, preprocessing_demo, list_datasets}` API across all 7 datasets (Webots/MSILN/IMUWiFine/IPIN floor 0/RoNIN canonical/TartanAir hospital/UJI); each per-dataset module surfaces honest `known_caveats` (IMUWiFine test no IMU, IPIN small-train, MSILN path-130 composition, RoNIN raw +94% gate, TartanAir image-only, UJI K=1 M=1 degenerate, Webots GPR-synthesised). New `src.pipeline.visualization` with 6 plotters (`plot_dataset_overview`/`plot_per_trajectory`/`plot_staleness_curve`/`plot_subset_eval_bar`/`plot_main_results_heatmap`/`plot_preprocessing_demo`) + stable color palette. **Smoke verification 7/7 datasets in ~13 s; 19 PNGs saved**. `docs/DATA_AND_VISUALIZATION.md` written; `CLAUDE.md` Pipeline Architecture table extended)
