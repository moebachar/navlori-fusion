...
- `CURRENT_ITERATION:` 23  (awaiting PLAN_23)
- `LAST_PLAN:` PLAN_22_main-results-ipin-floor0.md (IPIN 2024 floor 0 row: CNN1D + LSTM-attn + wlan_localization + RoNIN ResNet1D; pre-flight IMU-availability check per PLAN_20 lesson; third data point on LSTM-attn dead-reckoning replication)
- `LAST_RESULT:` RESULT_22_main-results-ipin-floor0.md (**IPIN floor 0 row populated, outcome β5**: wlanloc NEW val **20.53** / test 19.80 m; RoNIN ResNet1D NEW val 37.21 / test 31.70 m (IMU on all splits, unlike IMUWiFine); CNN1D fusion val 21.61 / test 20.45 m (0.34 M params); LSTM-attn fusion val 22.45 / test 21.56 m (0.41 M params). We BEAT RoNIN by ~40% val/35% test (decisive), LOSE to wlanloc by 5-9 % full-fusion. **Crucial diagnostic**: CNN1D `only:wifi` val 19.45 BEATS wlanloc val 20.53 by 5% — fusion regression is a small-train-overfit artifact (174 WiFi scans + 6924 IMU windows train, ~10× smaller than IMUWiFine), not fundamental WiFi failure. **LSTM-attn dead-reckoning regime REPLICATES on third dataset**: val only:imu 22.64 ≈ full 22.45 (1% gap); test only:imu 21.66 ≈ full 21.56 (1.3% gap). Combined with Webots+IMUWiFine, 3-dataset structural finding confirmed. Smoothness r=0.067 (CNN1D) / 0.089 (LSTM-attn) — highest in run-2 but still below 0.20 gate. PLAN_23 recommendation = RoNIN single-modality IMU row using RESULT_07's canonical ResNet1D 5.140 m as SOTA; expected: CNN1D/LSTM-attn IMU-only +90-94% gap)
- `GOAL_REACHED:` false
