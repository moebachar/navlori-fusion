...
- `CURRENT_ITERATION:` 21  (awaiting PLAN_21)
- `LAST_PLAN:` PLAN_20_val-test-gap-audit.md (user-flagged 2026-05-26 ~08:30 local: IMUWiFine val 1.397 / test 7.094 m = 5× gap (RESULT_19), inconsistent with Webots (+10-20%) and inverted vs MSILN (val>test). Audit splits/methodology across all measured datasets BEFORE more main-table rows. PLAN_21 resumes main-table at IPIN floor 0 if audit reveals documented dataset property; fix-first if real bug)
- `LAST_RESULT:` RESULT_20_val-test-gap-audit.md (**failure mode 3 — legitimate cross-session/cross-campaign dataset shift, NO code bug**. Decisive evidence: wlan_localization SOTA shows the same +104% gap on IMUWiFine — rules out any our-code culprit since SOTA uses separate pipeline. IMUWiFine's `convert_imuwifine.py:42-52` explicitly documents "two raw formats coexist" train/val (DATA_*.txt, Android logger) vs test (test_*.txt, no IMU, different timestamps); converter handles correctly per-file. Distribution probe: test paths have WiFi @ **5.65 Hz vs train+val 0.31 Hz (18× faster)**, **NO IMU**, GT y-range 1.2-1.6 m vs train+val 0-5 m (test walks a thin physical strip while train+val cover full corridor). Per-path CNN1D test MAE long-tailed 1.96-17.10 m across 20 paths; aggregate test median 3.62 m (close to val 1.40) but mean 7.09 m (dragged by 5 hard paths). Methodology sound; no fix needed. PLAN_21 recommendation = continue main-results table at IPIN floor 0; if IPIN val/test gap is similarly large, audit it too (but IPIN per CLAUDE.md is single-campaign so unlikely))
- `GOAL_REACHED:` false
