"""PLAN_13 — K=4 4-mod at B=128 (batch×lr confound probe).

Two coupled changes from RESULT_11:
  (i)  cfg.temporal.n_instants 8 → 4   (K-sweet-spot probe)
  (ii) drop odom_raw                  (unattended at K=1, distracting at K=8)

Modalities return to the run-1-documented 4-mod stack:
WiFi + IMU + Camera + Odom (single OdomCNN-P-B slot).

Staleness probe inherited from PLAN_11.

Run: ``.venv/Scripts/python.exe scripts/_train_webots_4mod_K4.py``
"""
...
OUT_DIR = ROOT / "runs" / "overnight" / "run2_iter_13"
...
def main():
    ...
    print("=== build config (simulation, 4-mod, K=4) ===", flush=True)
    cfg = load_config("simulation")
    cfg.dataset.modalities = ["wifi", "imu", "camera", "odom"]
    cfg.temporal.n_instants = 4   # halve from RESULT_11's K=8
    ...
    print("\n=== build encoders + vision tokens (4-mod, no odom_raw) ===", flush=True)
    encs, vision = build_encoders(cfg, dm)
    # NOTE: odom_raw is NOT registered this iteration — drop it per PLAN_12.
    ...
    # === memory probe ===
    print("\n=== memory budget probe (K=4) ===", flush=True)
    ...
    if peak_mb > 6000:
        raise RuntimeError(f"K=4 peak {peak_mb} MB exceeds 6 GB budget — try K=2")
    ...
    for p in [15, 16, 17]:
        ...
        if mask.sum() > 5:
            plot_path(pred[mask], gt[mask], p,
                      OUT_DIR / "test_paths" / f"K4_path_{p:02d}.png",
                      "(4-mod K=4)")
    ...
    with open(OUT_DIR / "wifi_imu_camera_odom_K4_B128.json", "w") as f:
        json.dump(out, f, indent=2)
    print(f"\nwrote {OUT_DIR / 'wifi_imu_camera_odom_K4_B128.json'}", flush=True)
