"""PLAN_22 Step 1a — wlan_localization on IPIN 2024 floor 0 (NEW measurement).

Clone of `_eval_wlanloc_imuwifine.py` with IPIN floor 0 paths and the
split from `configs/data/ipin2024_floor0.yaml`:
  train = paths 0-5, val = paths 6-9, test = paths 10-15.

Vendored `PositionRegressor` + `DataPreprocessor` loaded via `importlib`
per Demand #3.

Run: ``.venv/Scripts/python.exe scripts/_eval_wlanloc_ipin_floor0.py``
"""
...
DATA = ROOT / "data" / "ipin2024_floor0"
OUT_DIR = ROOT / "runs" / "overnight" / "run2_iter_22"
WLANLOC_SRC = Path(r"C:\Users\FabLab\AppData\Local\Temp\wlan_localization\src")

TRAIN_PATHS = [0, 1, 2, 3, 4, 5]
VAL_PATHS = [6, 7, 8, 9]
TEST_PATHS = [10, 11, 12, 13, 14, 15]
...
def main():
    ...
    print("Loading IPIN 2024 floor 0 WiFi...", flush=True)
    ...
    out["method"] = "wlan_localization PositionRegressor (k=3, manhattan, distance-weighted, IPIN 2024 floor 0)"
    out["preprocessor"] = "DataPreprocessor (vendored, fit on train)"
    with open(OUT_DIR / "wlanloc_ipin_floor0.json", "w") as f:
        json.dump(out, f, indent=2)
    print(f"\nwrote {OUT_DIR / 'wlanloc_ipin_floor0.json'}", flush=True)
