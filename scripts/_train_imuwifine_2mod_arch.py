"""PLAN_19 Step 2 — train CNN1D / LSTM-attn fusion on IMUWiFine floor 4 (WiFi+IMU only).

Adapted from `scripts/_train_webots_4mod_arch.py` (RESULT_17 wrapper).
Changes:
  - dataset = imuwifine (configs/data/imuwifine.yaml)
  - modalities = [wifi, imu]  (IMUWiFine has no camera/odom)
  - same K=4 + B=128 + lr=1.3e-3 + 90 epochs + dropouts as RESULT_17.

Run: ``.venv/Scripts/python.exe scripts/_train_imuwifine_2mod_arch.py --arch cnn1d``
"""
...
def main():
    ...
    out = {
        ...
        "latency_ms_per_sample_b1_estimate": float(lat_ms),
