"""PLAN_11 — K=8 temporal fusion on the 5-modality stack.

Same data + encoders as PLAN_10 (WiFi + IMU + Camera + Odom +
Odom_raw); flips `cfg.temporal.n_instants` from 1 → 8 and keeps
the run-1 audit-fix `instant_dropout=0.45` (already in
`configs/stage_c/fusion.yaml`).

Adds a **staleness probe** (Step 4a): re-evaluates test MAE while
the WiFi token is held stale by 0/3/7/12/20 instants (which at
`instant_stride=9` ≈ 0/3/7/12/20 × 0.9 s WiFi lag). The shape of
test MAE vs WiFi lag is the headline (K=1 cliff vs K>1 slope per
CLAUDE.md run-1 finding).

Run: ``.venv/Scripts/python.exe scripts/_train_webots_5mod_K8.py``
"""
...
OUT_DIR = ROOT / "runs" / "overnight" / "run2_iter_11"
...
def build_odom_raw_features(dm) -> dict[str, torch.Tensor]:
    ...
    for split, ds in [("train", dm.train_ds), ("val", dm.val_ds), ("test", dm.test_ds)]:
        ...
        for i, r in enumerate(ds._gt_rows):
            ...
            feats[i, 0] = float(np.interp(t, ts, xs)) - xs[0]
            feats[i, 1] = float(np.interp(t, ts, ys)) - ys[0]
        out[split] = torch.tensor(feats, dtype=torch.float32).unsqueeze(1)
    return out
...
def staleness_probe(trainer: FusionTrainer, lags: list[int]) -> dict:
    """Re-eval test MAE while holding WiFi token stale by `lag` instants.

    At instant_stride=9 and 10 Hz GT, one instant ≈ 0.9 s. Lag of 5
    instants ≈ 4.5 s of stale WiFi. The probe replaces the WiFi
    feature at every instant with the WiFi feature `lag` instants
    earlier (effectively held constant for `lag` × 0.9 s).
    """
    if "wifi" not in trainer.modalities:
        return {}
    K = trainer.n_instants
    out = {}
    trainer.model.eval()
    orig = trainer.X["test"]["wifi"]
    # Build cumulative indices arrays once per split so we can index per-sample.
    for lag in lags:
        if lag == 0:
            trainer.X["test"]["wifi"] = orig
            # Eval normally.
        else:
            # Shift the wifi cache backward in time by `lag` indices along axis 0.
            # The cache is (N_global, window, n_aps); shifting moves each
            # sample's reading to be `lag` global-index positions earlier.
            shifted = orig.clone()
        ...
        out[lag] = mae
        print(f"  WiFi lag={lag} instants ({lag * 0.9:.1f} s): test MAE = {mae:.3f} m",
              flush=True)
    trainer.X["test"]["wifi"] = orig  # restore
    return out
...
def main():
    ...
    ap.add_argument("--batch-size", type=int, default=64)  # K=8 → halve from 128
    ap.add_argument("--pretest-epochs", type=int, default=5)
    args = ap.parse_args()

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    print("=== build config (simulation, 5-mod, K=8) ===", flush=True)
    cfg = load_config("simulation")
    cfg.dataset.modalities = ["wifi", "imu", "camera", "odom"]
    # Keep the fusion.yaml defaults: temporal.n_instants=8, instant_stride=9.
    ...
    print("\n=== build encoders + vision tokens + odom_raw ===", flush=True)
    ...
    print(f"  odom_raw: train {extra['odom_raw']['train'].shape}", flush=True)
    ...
    # === memory probe ===
    print("\n=== memory budget probe (K=8) ===", flush=True)
    ...
    if peak_mb > 6000:
        raise RuntimeError(f"K=8 peak {peak_mb} MB exceeds 6 GB budget — try K=4")
    ...
    # === staleness probe (WiFi lag) ===
    print("\n=== WiFi staleness probe ===", flush=True)
    staleness = staleness_probe(trainer, lags=[0, 3, 7, 12, 20])
    ...
    for p in [15, 16, 17]:
        ...
        if mask.sum() > 5:
            plot_path(pred[mask], gt[mask], p,
                      OUT_DIR / "test_paths" / f"K8_path_{p:02d}.png",
                      "(5-mod K=8)")
    ...
    with open(OUT_DIR / "wifi_imu_camera_odom_K8.json", "w") as f:
        json.dump(out, f, indent=2)
    print(f"\nwrote {OUT_DIR / 'wifi_imu_camera_odom_K8.json'}", flush=True)
